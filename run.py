...
def count_pv(dt,hm,adddd = True):
    if os.path.exists("pv.json") == False:
...
def new(dark):
    ...
    if regoin == 'CN' or regoin == 'None':
        if (city == 'Jinan') or (city == 'None') :
            ...
            hint += "我们是齐鲁工业大学（长清校区）的空教室查询工具，如果您需要查询你们大学的空教室的话，可以参考下方的github仓库自行部署 。<br>"
    elif regoin is None:
        pass
    else:
        hint += "内网穿透服务提供商是cloudflare，我们建议您使用直连的方式访问本站哦~<br>"\

    if Proto == 'http':
        hint += "你知道么？其实我们支持https访问哦！ <a href = 'https://qlu.nixiak.xyz'>点我跳转</a> 。<br>"
    host = request.headers.get('Host')
    ua = request.headers.get('User-Agent')

    if host != 'qlu.nixiak.xyz':
        hint += "你知道么？我们的域名是qlu.nixiak.xyz哦！ <a href = 'https://qlu.nixiak.xyz'>点我跳转</a> .  此链接可以支持校外网络访问哦~"
        ip = request.remote_addr
    with open("./static/data/userip.json", "a") as f:
        f.write('\n'+dt+"--"+hm+"--"+ip)
    if ua.find('MicroMessenger') != -1:
        hint += "亲，这边建议您不要使用微信直接点击链接访问本站哦，复制链接到浏览器访问性能更好呦~   https://qlu.nixiak.xyz 。<br>"
    ...
    weeks,week_i=school_schedule()
    if dark == 'dark':
        return render_template("index.html",dark=dark,def_week=weeks,def_day=week_i)
    else:
        return render_template("index.html",def_week=weeks,def_day=week_i)
