...
import pytz
'''逻辑分析'''
#get_table:获取到所有的课表数据，包括教室以及相关的教师课程及周次
#get_course_on_table:返回所有的教室（多层字典），返回教室名
#cell_parse:保存一下相关的数据
#week_name_parse:去除其他数据，只保留教室名称
...
def get_table():
    # 内网使用可用url1，非校园网的情况下请使用vpn
    url1 = 'http://jwxt.qlu.edu.cn/jsxsd/kbcx/kbxx_classroom_ifr'
    url2 = "http://jwxt-qlu-edu-cn.vpn.qlu.edu.cn/jsxsd/kbcx/kbxx_classroom_ifr"
    ...
    result = html.xpath('//tr')  #element对象的列表形式，
    table = []
    # 从第三行开始,前两行是星期和节数
    #去掉相关的网页标签元素
...
class JsonEncoder(json.JSONEncoder):
    def default(self, obj, MyEncoder=None):
